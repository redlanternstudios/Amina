...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  return NextResponse.json({
    post: { id: post.id, content: post.content_text, is_anonymous: post.is_anonymous, created_at: post.created_at, display_handle: 'Sister', has_reacted: false, is_mine: true }
  }, { status: 201 })
}
