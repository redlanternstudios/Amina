  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const { data: c, error: cErr } = await supabase
        .from('circles').select('id, name, description, avatar_url, is_public, creator_id')
      ...
      if (!membership && c.is_public === false) {
        setError('You are not a member of this circle'); setLoading(false); return
      }

      setUserRole(membership?.role || null)

      const { count } = await supabase
        .from('circle_memberships').select('*', { count: 'exact', head: true })
        .eq('circle_id', circleId).eq('status', 'active')

      if (!mounted) return
      setCircle({ ...c, member_count: count || 0 } as CircleData)

      const { data: mems } = await supabase
        .from('circle_memberships').select('id, user_id, role, circle_profiles!inner(display_name, avatar_url)')
        .eq('circle_id', circleId).eq('status', 'active')
      ...
      <div className="bg-white border-b border-charcoal/10 px-4 py-3">
        <div className="flex items-center gap-3 mb-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-charcoal/40">{circle.member_count} members</p>
          </div>
        </div>
        {circle.description && (<p className="text-sm text-charcoal/70 ml-11">{circle.description}</p>)}
      </div>

      <div className="flex border-b border-charcoal/10 bg-white">
        {tabs.map(tab => (
          <button key={tab.key} onClick={() => setActiveTab(tab.key)}
            className={`flex-1 py-3 text-xs font-medium text-center transition-all ${activeTab === tab.key ? 'text-rose-amina border-b-2 border-rose-amina' : 'text-charcoal/50 hover:text-charcoal'}`}>{tab.label}</button>
        ))}
      </div>
      ...
      <div className="pb-24">
        {activeTab === 'posts' && (
          <div className="text-center py-16">
            ...
            <button onClick={() => router.push(`/circle/${circleId}/posts`)} className="mt-3 bg-rose-amina text-white text-xs font-semibold px-4 py-1.5 rounded-full">Go to Posts</button>
          </div>
        )}
        {activeTab === 'chat' && (
          <div className="text-center py-16">
            ...
            <button onClick={() => router.push(`/circle/${circleId}/chat`)} className="mt-3 bg-rose-amina text-white text-xs font-semibold px-4 py-1.5 rounded-full">Open Chat</button>
          </div>
        )}
        {activeTab === 'members' && (
            ...
            {members.length === 0 ? (
              ...
              members.map(member => (
                <div key={member.id} className="flex items-center gap-3 bg-ivory rounded-xl px-4 py-3">
                  ...
                  {member.role === 'creator' && <span className="text-[10px] bg-rose-100 text-rose-dark px-2 py-0.5 rounded-full font-medium">Creator</span>}
                  {member.role === 'admin' && <span className="text-[10px] bg-amber-100 text-amber-700 px-2 py-0.5 rounded-full font-medium">Admin</span>}
                </div>
              ))
            )}
          </div>
