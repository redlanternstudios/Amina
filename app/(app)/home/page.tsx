...
const QUICK_CHIPS = [
  { id: 'reflect', icon: '❤️', label: 'Reflect', message: 'Help me reflect on something today.' },
  { id: 'guidance', icon: '🌙', label: 'Guidance', message: 'I need some guidance today.' },
  { id: 'learn', icon: '📖', label: 'Learn', message: 'Help me learn something about my faith.' },
  { id: 'grow', icon: '🌿', label: 'Grow', message: 'Help me grow spiritually.' },
]
...
export default function HomePage() {
  ...
  function handleSend() {
    ...
    router.push(`/chat/${id}?q=${encodeURIComponent(message.trim())}`)
  }
        ...
        <div className="relative rounded-3xl overflow-hidden bg-ivory min-h-[160px] flex flex-col justify-end p-5">
          ...
          <div className="flex gap-2 mt-3 overflow-x-auto pb-1 -mx-1 px-1">
            {QUICK_CHIPS.map(chip => (
              <button
                key={chip.id}
                onClick={() => router.push(`/chat/${crypto.randomUUID()}?q=${encodeURIComponent(chip.message)}`)}
